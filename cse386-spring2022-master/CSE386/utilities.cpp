...
double normalizeDegrees(double degrees) {
	/* CSE 386 - todo  */
	return glm::mod<float>(degrees,360);
}
...
double normalizeRadians(double rads) {
	/* CSE 386 - todo  */
	return glm::mod<float>(rads, 2*PI);
}
...
double areaOfTriangle(double x1, double y1, double x2, double y2, double x3, double y3) {
	double a = sqrt(glm::pow((x1 - x2), 2) + glm::pow(y1 - y2, 2));
	double b = sqrt(glm::pow((x1 - x3), 2) + glm::pow(y1 - y3, 2));
	double c = sqrt(glm::pow((x3 - x2), 2) + glm::pow(y3 - y2, 2));
	double s = (a + b + c) / 2;
	double answer = sqrt(s * (s - a) * (s - b) * (s - c));
	return answer;
}
...
dvec2 pointOnCircle(const dvec2& center, double R, double angleRads) {
	...
	return dvec2(R*glm::cos(angleRads) + center.x, R*glm::sin(angleRads) + center.y);
}
...
* @test fix with the canvas announcement
*/

double directionInRadians(const dvec2& referencePt, const dvec2& targetPt) {
	/* CSE 386 - todo  */
	double top = targetPt.y - referencePt.y;
	double bottom = targetPt.x - referencePt.x;
	if (top == 0 && bottom > 0) {
		return 0;
	}
	...
	if (top > 0 && bottom > 0)
	{
		return glm::atan(top / bottom);
	}
	else if (top > 0 && bottom < 0) {
		//return (-1 * glm::degrees(glm::atan(bottom / top))) + (90);
		return ( -1 * glm::atan(bottom / top)) + (PI / 2);
	}
	else if (top < 0 && bottom < 0) {
		//return (glm::degrees(glm::atan(top / bottom))) + 180;
		return (glm::atan(top / bottom)) + PI;
	}
	else if (top < 0 && bottom > 0) {
		//return ((glm::degrees(glm::atan(bottom / top)))) + 270;
		return (-1 * (glm::atan(bottom / top))) + (1.5 * PI);
	}
